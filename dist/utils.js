...
exports.convertPathToMswPattern =
	exports.generateErrorResponse =
		...
		case "array": {
			if (schema.items) {
				...
				return Array.from({ length }, () =>
					(0, exports.generateRandomValue)(schema.items, locale),
				);
			}
			return [];
		}
...
const generateMockData = (schema, openApiSchema, locale = "en") => {
			...
			(acc, [key, value]) => {
				...
				if (resolvedValue.type === "array" && resolvedValue.items) {
					if (resolvedValue.example) {
						...
						const itemsSchema = (0, exports.resolveRef)(
							resolvedValue.items,
							openApiSchema,
						);
						const length = faker_1.faker.number.int({ min: 1, max: 5 });
					...
					acc[key] =
						resolvedValue.example ??
						(0, exports.generateRandomValue)(resolvedValue, locale);
				}
				return acc;
			},
			{},
	...
	if (resolvedSchema.type === "array" && resolvedSchema.items) {
		...
		const resolvedItems = (0, exports.resolveRef)(
		...
		if (
			...
			return Array.from({ length }, () => {
				...
				let id;
				do {
					item = (0, exports.generateMockData)(
						resolvedItems,
						openApiSchema,
						locale,
					);
